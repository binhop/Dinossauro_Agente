...
class Dino(pygame.sprite.Sprite): 
    ...
    """ Ações do jogador """
    def action(self):
        #on_ground = int(self.rect.y >= SCREEN_HEIGHT - self.rect.height - GND_HEIGHT)
        ...
        x = [self.speed, dist, self.sensors[1], self.sensors[2]]
...
class Screens():
    ...
    def create_run_elements(self, agents_size):
        ...
        self.active_sprite_list.add(self.ground, self.cactus)

        # O fator de mutação começa em 1.0 e a cada 10 gerações
        # ele cai pela metade
    ...
    def screen_run(self, events):        
        ...
        score_text = FONT_SCORE.render("%05d"%self.score, True, (0, 0, 0))
        self.screen.blit(score_text, (SCREEN_WIDTH - score_text.get_rect().width, 0))

        # Escreve o texto do framerate
        frate_text = FONT_SCORE.render("FPS: %03d"%self.framerate, True, (0, 0, 0))
        self.screen.blit(frate_text, (0, 20))

        # Escreve o texto da geração atual
        if self.cur_screen == "RunTrain":
            gen_text = FONT_SCORE.render("Geração: %d"%self.train_generation, True, (0, 0, 0))
            self.screen.blit(gen_text, (220, 0))

            # Escreve o texto do fator de mutação
            mut_text = FONT_SCORE.render("Fator de mutação: %.4f"%self.mutation_factor, True, (0, 0, 0))
            self.screen.blit(mut_text, (220, 20))
            
            # Escreve o texto do auto avanço
            avan_text = FONT_SCORE.render("Automático: %s"%('x' if self.auto_advance else '-'), True, (0, 0, 0))
        ...
        alive_text = FONT_SCORE.render(str("Dinos vivos: %02d/%02d"%(dinos_alive,len(self.dinos))), True, (0, 0, 0))
